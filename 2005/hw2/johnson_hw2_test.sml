use "johnson_hw2.sml";
val name_hw = "2005 - Assignment Two"
val f1 = find_playable
val f2 = without_bone
val f3 = layout_summary
val f4 = best_move
val f5 = do_move
	     
(*Unit tests are of format string*bool where bool is represented by evaluation of a function to an expected value*)
val tests = [
    ("1.0", f1([], 3) = NONE),
    ("1.1", f1([(1,1)], 1) = SOME (1,1)),
    ("1.2", f1([(1,1)], 2) = NONE),
    ("1.3", f1([(1,1),(2,1),(3,4)], 1) = SOME (1,1)),
    ("1.4", f1([(1,1),(2,1),(3,4)], 2) = SOME (2,1)),
    ("1.5", f1([(1,1),(2,1),(3,4)], 3) = SOME (3,4)),
    ("1.6", f1([(1,1),(2,1),(3,4)], 4) = SOME (3,4)),
    ("1.7", f1([(1,1),(2,1),(3,4)], 5) = NONE),

    ("2.0", f2([],(1,3)) = []),
    ("2.1", f2([(2,3)],(2,3)) = [] andalso f2([(3,2)],(2,3)) = [] andalso f2([(2,3)],(3,2)) = []),
    ("2.2", f2([(5,6),(3,4),(1,2)],(5,6)) = [(3,4),(1,2)] andalso f2([(5,6),(3,4),(1,2)],(3,4)) = [(5,6),(1,2)] andalso f2([(5,6),(3,4),(1,2)],(1,2)) = [(5,6),(3,4)]),

    ("2.3", f2([(5,6),(3,4),(1,2),(5,6)],(5,6)) = [(3,4),(1,2),(5,6)] andalso f2([(5,6),(3,4),(3,4),(1,2)],(3,4)) = [(5,6),(3,4),(1,2)] andalso f2([(5,6),(1,2),(3,4),(1,2)],(1,2)) = [(5,6),(3,4),(1,2)]),

    ("3.0", f3([]) = NONE),
    ("3.1", f3([(1,7)]) = SOME (1,7)),
    ("3.2", f3([(1,1),(2,1)]) = SOME (1,1)),
    ("3.3", f3([(1,1),(2,1),(3,4)]) = SOME (1,4)),
    ("3.4", f3([(5,6),(3,4),(1,2),(5,6)]) = SOME (5,6)),

    ("4.0", f4([],[]) = PassDraw),
    ("4.1", f4([],[(1,2)]) = PlayFirst (1,2)),
    ("4.2", f4([(1,2)],[(1,2)]) = PlayLeft (1,2)),
    ("4.3", f4([(2,1)],[(1,2)]) = PlayLeft (1,2)),
    ("4.4", f4([(3,4),(4,6)],[(3,2)]) = PlayLeft (3,2)),
    ("4.5", f4([(3,4),(4,6)],[(1,3)]) = PlayLeft (1,3)),
    ("4.6", f4([(3,4),(4,6)],[(2,6)]) = PlayRight (2,6)),
    ("4.7", f4([(3,4),(4,6)],[(6,5)]) = PlayRight (6,5)),
    ("4.8", f4([(5,6),(3,4),(1,2),(3,4)],[(7,7)]) = PassDraw),

    (*a layout l, a deck d, a hand h, and a move m*)
    ("5.00", f5([],[],[],PassDraw) = ([],[],[])),
    ("5.01", f5([(1,2)],[],[],PassDraw) = ([(1,2)],[],[])),
    ("5.02", f5([],[],[(1,2)],PassDraw) = ([],[],[(1,2)])),
    ("5.03", f5([],[(1,2)],[],PassDraw) = ([],[],[(1,2)])),
    ("5.04", f5([],[(1,2),(3,4)],[],PassDraw) = ([],[(3,4)],[(1,2)])),

    ("5.10", f5([(2,3)],[],[(4,5)],PlayFirst(4,5)) = ([],[],[]) handle BadMove => true),
    ("5.11", f5([],[],[(3,6)],PlayFirst(4,5)) = ([],[],[]) handle BadMove => true),
    ("5.12", f5([],[],[(4,5)],PlayFirst(4,5)) = ([(4,5)],[],[])),

    ("5.20", f5([],[],[(4,5)],PlayRight(4,5)) = ([],[],[]) handle BadMove => true),
    ("5.21", f5([],[],[(4,5)],PlayLeft(4,5)) = ([],[],[]) handle BadMove => true),
    ("5.22", f5([(4,5)],[],[(4,5)],PlayRight(6,7)) = ([],[],[]) handle BadMove => true),
    ("5.23", f5([(4,5)],[],[(4,5)],PlayLeft(6,7)) = ([],[],[]) handle BadMove => true),
    ("5.24", f5([(4,5)],[],[(3,2)],PlayRight(3,2)) = ([],[],[]) handle BadMove => true),
    ("5.25", f5([(4,5)],[],[(3,2)],PlayLeft(3,2)) = ([],[],[]) handle BadMove => true),
    ("5.26", f5([(4,3)],[],[(3,2)],PlayRight(3,2)) = ([(4,3),(3,2)],[],[])),
    ("5.27", f5([(4,2)],[],[(3,2)],PlayRight(3,2)) = ([(4,2),(2,3)],[],[])),
    ("5.28", f5([(4,3)],[],[(5,4)],PlayLeft(5,4)) = ([(5,4),(4,3)],[],[])),
    ("5.29", f5([(4,3)],[],[(4,5)],PlayLeft(4,5)) = ([(5,4),(4,3)],[],[])),
    ("5.291", f5([(4,3)],[],[(2,3)],PlayRight(3,2)) = ([(4,3),(3,2)],[],[])),
    ("5.292", f5([(4,2)],[],[(2,3)],PlayRight(3,2)) = ([(4,2),(2,3)],[],[])),
    ("5.293", f5([(4,3)],[],[(4,5)],PlayLeft(5,4)) = ([(5,4),(4,3)],[],[])),
    ("5.294", f5([(4,3)],[],[(5,4)],PlayLeft(4,5)) = ([(5,4),(4,3)],[],[])),

    ("5.30", f5([(1,2)],[],[(5,2),(3,0)],PlayRight(5,2)) = ([(1,2),(2,5)],[],[(3,0)]))

	
];

print ("\n----------------------"^name_hw^"--------------------------\n");
fun all_tests(tests) =
	let fun helper(tests: (string*bool) list, all_passed) = 
		case tests of
			[] => all_passed
	  	  | (st, true)::rest =>  helper(rest, all_passed) (*I am only interested in the output if the test fails.*)
	  	  | (st, false)::rest => (print ("***" ^ st ^" !!!FAILED!!!\n"); helper(rest, false))
	in
		helper(tests, true)
	end;

case all_tests(tests) of
    true => print "--------------EVERY TESTS PASSED-------------\n"
  | false => print "--------------SOMETHING IS WRONG-------------------------\n";
